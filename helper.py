...
def meff(Corr_est:np.ndarray, Corr_err:np.ndarray, Corr_bst:np.ndarray, delta:float = 1):
    r"""
    ...
    if Corr_bst is None:
        ...
        meff_gvar = (gv.log(np.roll(Corr_gvar,-1,axis=0)) - gv.log(np.roll(Corr_gvar,1,axis=0))) / (2 * delta)

        return None, gv.mean(meff_gvar), gv.sdev(meff_gvar)

    else:
        # No bootstrap time axis assumed in first axis
        meff_bst = (np.log(np.roll(Corr_bst,-1,axis=1)) - np.log(np.roll(Corr_bst,1,axis=1))) / (2 * delta)
