...
def fit(
    *,
    ...
    args["fcn"] = model
    args["maxit"] = maxiter
    if prior is None and po is None:
        raise ValueError(f"at least one of prior and po needs to be defined")
...
if __name__ == "__main__":
    ...
    for nbst in range(Nbst):
        data_bst[nbst] = np.mean(
            data[np.random.randint(0, Nconf, size=(Nconf,))], axis=0
        )
