"""
example_hybrid_variable_projection_2exp.py
======================
Minimal example: two-exponential correlator fit via the hybrid + variable projection backend.
...
fit_result = fit(
    backend                      = "hybrid:adam+iminuit",
    abscissa                     = t,
    ordinate                     = ordinate,
    model                        = model_2exp,
    # providing linear params triggers variable projection 
    linear_params                = ["A0", "A1"], 
    # prior                        = prior,
    p0                           = p0,
    # We can make the energy params explicitly positive
    limits                       = {
        "E0": (0,None), "dE1": (0.1,None)
    },
...
fig.suptitle("Two-Exponential Correlator Fit  (hybrid + variable projection backend)", fontsize=14, y=0.98)
...
plt.savefig("plot_hybrid_variable_projection_2exp.pdf", bbox_inches="tight")